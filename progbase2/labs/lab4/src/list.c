...
void List_set(List* self, char* data, int position) {
	University_set(List_getUniversity(self, position), data);
}
...
void List_findUniversity(List* head, const char* location) {
	University* cur = head->items;
	int found = 0;
	while(cur != NULL) {
		if(strcmp(location, University_getLocation(cur)) == 0) {
			University_print(cur);
			found++;
		}
		cur = University_getNext(cur);
	}
	if(!found) printf("There's no universities from %s", location);
}
