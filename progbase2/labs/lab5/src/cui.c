...
void Cui_inHelp(void){
     conMove(MID - 1,1);
     printf("HELP: Choose one of the command from the list");
     conReset();
     conMove(MID + 1,1);
}

void Cui_clearImage(void) {
...
void Cui_UniversityFields(void)
{
    ...
    printf("STRUCT: name, location, rank, matanPoints, students");
    conReset();
    conMove(MID + 1,1);
}
...
int FormatDetector(const char* filename) {
